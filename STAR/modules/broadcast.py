...
from STAR.config import SUDO_USERS
from STAR.utils.filters import command
from STAR.tgcalls import client as USER
from STAR.database.chats import add_served_chat, blacklisted_chats, get_served_chats
...
@app.on_message(command("اذاعة بالتثبيت") & filters.user(SUDO_USERS))
async def broadcast_message(_, message):
    ...
    if len(message.command) < 2:
        await message.reply_text("- الاستخدام : اذاعة بالتثبيت بالرد على مسج")
        return
...
async def broadcast_message(_, message):
    ...
    for i in chats:
        try:
            ...
            pass
    await m.edit(f" **- اذاعة الرسالة {sent} جروب .**")
