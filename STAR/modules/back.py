...
from STAR import BOT_NAME, BOT_USERNAME
...
dbclean_keyboard = InlineKeyboardMarkup(
    [
        [
            ...
            InlineKeyboardButton("حذف الكل", callback_data="cleandb"),],
        [    InlineKeyboardButton("لا ارجع", callback_data="cbmenu"),
...
menu_keyboard = InlineKeyboardMarkup(
    [
        [
            ...
            InlineKeyboardButton(text="‹ الغاء الكل ›", callback_data=f"dbconfirm"),
             InlineKeyboardButton(text="‹ الدعم ›", callback_data=f"nonabout"),
        ],[
             InlineKeyboardButton(text="‹ الغاء ›", callback_data=f"cls"),
        ],
    ]
)
...
async def pausevc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.answer(f"- مفيش حاجة شغالة ", show_alert=True)
            return
    else:
        await CallbackQuery.answer(f"- مفيش حاجة شغالة !", show_alert=True)
...
""",
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer(f"- ورحمة جدك مفيش حاجة شغالة . ", show_alert=True)
...
async def stopvc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer(f"- مفيش حاجة شغالة متخلينيش اكفر", show_alert=True)
...
async def cleandb(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer(f"- متكفرنيش مفيش حاجة شغالة .", show_alert=True)
...
async def cbmenu(_, query: CallbackQuery):
    if query.message.sender_chat:
        return await query.answer("عايزه **راجل** ههه امزح لازم تكون ادمن")
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer("‹ عايزه **راجل** ههه امزح لازم تكون ادمن ›", show_alert=True)
    chat_id = query.message.chat.id
    if is_music_playing(chat_id):
          await query.edit_message_text(
              f"**‹ مرحبا بك في قائمة الاعدادات ›\n\n- أسم جࢪۅبي : {query.message.chat.title}.\n- أيدي جࢪۅبي : {query.message.chat.id}\n\n**- يمكنك التحكم في الازرار ولكن كن حذراً .**",

              reply_markup=menu_keyboard
         )
    else:
        await query.answer("- مفيش حاجة شغالة. ", show_alert=True)
...
async def fifth(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer(f"- مفيش حاجه شغالة . ", show_alert=True)
...
async def fourth(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer("تم الصوت 150 !", show_alert=True)
        await CallbackQuery.edit_message_text(
...
async def third(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer("تم الصوت 100", show_alert=True)
        await CallbackQuery.edit_message_text(
        f"**‹ يمكنك التحكم في اعدادات حجم الصوت ›**\n\n- عند استخدامك لحجم الصوت عليك رفع حساب المساعد كمشرف هنا",
...
async def second(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer("تم الصوت 50", show_alert=True)
        await CallbackQuery.edit_message_text(
        f"**‹ يمكنك التحكم في اعدادات حجم الصوت ›**\n\n- عند استخدامك لحجم الصوت عليك رفع حساب المساعد كمشرف هنا.",
...
async def first(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        await CallbackQuery.answer("- تم الصوت 20!", show_alert=True)
        await CallbackQuery.edit_message_text(
        f"**‹ يمكنك التحكم في اعدادات حجم الصوت ›**\n\n- عند استخدامك لحجم الصوت عليك رفع حساب المساعد كمشرف هنا.",
